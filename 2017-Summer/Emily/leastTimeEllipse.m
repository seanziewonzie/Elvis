function varargout = leastTime2(varargin)

%
% Author: 
%   Zachary Bradshaw
%
%Notes:
%   Want to discretize original angle theta and keep track of parameters
%   use Snell's law at boundary
%
% Usage: 
%   leastTime  - function that creates gui & components
%   varargout - represents any number of output arguments
%   varargin  - represents any number of input arguments
% 
% [fig] - add initial figure
fig = figure(...
    'Tag',             'fig',...
    'Name',            'Least Time Interface',...
    'NumberTitle',     'off',...
    'Visible',         'on',... 
    'MenuBar',         'none',...
    'Units',           'normalized',...
    'UserData',        struct(... % use for passing params
                            'plotData',[],...
                            'resultData',[]...
                       ),...
    'Position',        [.1 .1 .75 .75]);



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Add Toolbar & Push Buttons                                   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% add toolbar
tb = uitoolbar('Parent',fig);

% get open icon png & convert to use in toolbar
[img_open,~,alpha] = imread(fullfile(matlabroot,...
    'toolbox/matlab/icons/file_open.png'));
openIcon = double(img_open)/256/256;
openIcon(~alpha) = NaN;

% get save icon png & convert to use in toolbar
[img_save,~,alpha] = imread(fullfile(matlabroot,...
    'toolbox/matlab/icons/file_save.png'));
saveIcon = double(img_save)/256/256;
saveIcon(~alpha) = NaN;

% get pan icon png & convert to use in toolbar
[img_save,~,alpha] = imread(fullfile(matlabroot,...
    'toolbox/matlab/icons/tool_hand.png'));
panIcon = double(img_save)/256/256;
panIcon(~alpha) = NaN;

% get zoom in icon png & convert to use in toolbar
[img_save,~,alpha] = imread(fullfile(matlabroot,...
    'toolbox/matlab/icons/tool_zoom_in.png'));
zoomInIcon = double(img_save)/256/256;
zoomInIcon(~alpha) = NaN;

% get zoom in icon png & convert to use in toolbar
[img_save,~,alpha] = imread(fullfile(matlabroot,...
    'toolbox/matlab/icons/tool_zoom_out.png'));
zoomOutIcon = double(img_save)/256/256;
zoomOutIcon(~alpha) = NaN;

% [openBtn] - open button
uipushtool(...
    'Parent',          tb,...
    'Tag',             'openBtn',...
    'CData',           openIcon,...
    'TooltipString',   'Open File',...
    'ClickedCallback', @openBtnCallback);

% [saveBtn] - save button
uipushtool(...
    'Parent',          tb,...
    'Tag',             'saveBtn',...
    'CData',           saveIcon,...
    'TooltipString',   'Save File',...
    'ClickedCallback', @saveBtnCallback);

% [panBtn] - pan button
uipushtool(...
    'Parent',          tb,...
    'Tag',             'panBtn',...
    'CData',           panIcon,...
    'TooltipString',   'pan tool',...
    'ClickedCallback', 'pan');

% [zoomInBtn] - zoom in button
uipushtool(...
    'Parent',          tb,...
    'Tag',             'zoomInBtn',...
    'CData',           zoomInIcon,...
    'TooltipString',   'zoom in tool',...
    'ClickedCallback', 'zoom');

% [zoomOutBtn] - zoom out button (just resets zoom settings)
uipushtool(...
    'Parent',          tb,...
    'Tag',             'zoomOutBtn',...
    'CData',           zoomOutIcon,...
    'TooltipString',   'zoom out to initial zoom setting',...
    'ClickedCallback', 'zoom out');

tgroup = uitabgroup('Parent',fig);

tab1 = uitab(...
    'Parent',          tgroup,...
    'Tag',             'tab1',...
    'Title',           'Circle');

tab2 = uitab(...
    'Parent',          tgroup,...
    'Tag',             'tab2',...
    'Title',           'Ellipse');

tab1pan = uipanel(...
    'Parent',           tab1,...
    'Tag',              'tab1Pan',...
    'Title',            'Circle',...
    'FontSize',         12,...
    'FontWeight',       'bold',...
    'Units',            'normalized',...
    'Position',         [.05 .05 .9  .95]);


tab2pan = uipanel(...
    'Parent',           tab2,...
    'Tag',              'tab2Pan',...
    'Title',            'Ellipse',...
    'FontSize',         12,...
    'FontWeight',       'bold',...
    'Units',            'normalized',...
    'Position',         [.05 .05 .9  .95]);

% [speed1 Static]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'r1Static',...
    'Style',           'text',...
    'String',          'Speed 1 :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .9 .1 .05]);

% [speed2 Static]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'r2Static',...
    'Style',           'text',...
    'String',          'Speed 2 :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .8 .1 .1]);

% [y Start Position Static]
uicontrol(...
    'Parent',         tab1pan,...
    'Tag',             'ystartPStatic',...
    'Style',           'text',...
    'String',          'Y Start Coordinate :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.6 .75 .15 .1]);

% [Time Static]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'timeStatic',...
    'Style',           'text',...
    'String',          'Time :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .7 .1 .1]);

% [Discrete Static]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'discStatic',...
    'Style',           'text',...
    'String',          'Disc. Factor :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.6 .65 .15 .1]);

% [SRS Discrete Static]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'SRSStatic',...
    'Style',           'text',...
    'String',          'SRS Disc. :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.6 .6 .15 .1]);

% [Time Slider Static]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'tSliderStatic',...
    'Style',           'text',...
    'String',          'Time Slider',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.725 .375 .15 .1]);

% [speed1]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'speed1',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .9 .15 .05]);

% [speed2]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'speed2',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .85 .15 .05]);

% [y start position]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'ystartP',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .8 .15 .05]);

% [time]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'time',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .75 .15 .05]);

% [discFactor]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'discFactor',...
    'Style',           'edit',...
    'String',          '150',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .699 .15 .05]);

% [SRSdiscFactor]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'SRS_disc',...
    'Style',           'edit',...
    'String',          '50',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .65 .15 .05]);

% [Time Slider]
uicontrol(...
    'Parent',          tab1pan,...
    'Tag',             'time_slider',...
    'Style',           'slider',...
    'Max',             15,...
    'Min',             1,...
    'Value',           5,...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .5 .15 .05],...
    'Callback',        @time_slider_Callback);
    

%run button
uicontrol(...
    'Parent',       tab1pan,...
    'Tag',             'runBtn',...
    'Style',           'pushbutton',...
    'String',          'Run Simulation',...
    'FontSize',        12,...
    'FontWeight',      'bold',...
    'Units',           'normalized',...
    'Position',        [.45 .05 .15 .1],...
    'Callback',        @run_button_Callback);

%zoom_box button
uicontrol(...
    'Parent',           tab1pan,...
    'Tag',             'zoomboxBtn',...
    'Style',           'pushbutton',...
    'String',          'Zoom in on Snells Area',...
    'FontSize',        10,...
    'FontWeight',      'bold',...
    'Units',           'normalized',...
    'Position',        [.75 .60 .15 .05],...
    'Callback',        @zoombox_button_Callback);

% [axes] - add axes for plot
axes(...
    'Parent',          tab1pan,...
    'Tag',             'axes',...
    'Units',           'normalized',...
    'Position',        [.05 .4 .5 .5]);

%[a1 Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'a1static',...
    'Style',           'text',...
    'String',          'a1:',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .85 .1 .05]);
%[b1 Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'b1Static',...
    'Style',           'text',...
    'String',          'b1:',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .8 .1 .05]);

% [a2 Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'a2Static',...
    'Style',           'text',...
    'String',          'a2:',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .7 .1 .1]);

%[b2 Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'b2Static',...
    'Style',           'text',...
    'String',          'b2:',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .7 .1 .05]);

% [y Start Position Static]
uicontrol(...
    'Parent',         tab2pan,...
    'Tag',             'ystartPStatic',...
    'Style',           'text',...
    'String',          'Y Start Coordinate :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.6 .6 .15 .1]);

% [Time Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'timeStatic',...
    'Style',           'text',...
    'String',          'Time :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.65 .55 .1 .1]);

% [Discrete Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'discStatic',...
    'Style',           'text',...
    'String',          'Disc. Factor :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.6 .5 .15 .1]);

% [SRS Discrete Static]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'SRSStatic',...
    'Style',           'text',...
    'String',          'SRS Disc. :',...
    'FontSize',        12,...
    'HorizontalAlignment', 'right',...
    'Units',           'normalized',...
    'Position',        [.6 .45 .15 .1]);


uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'a1',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .85 .15 .05]);

uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'b1',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .8 .15 .05]);

% [speed2]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'a2',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .75 .15 .05]);

uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'b2',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .7 .15 .05]);

% [y start position]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'ystartP2',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .65 .15 .05]);

% [time]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'time2',...
    'Style',           'edit',...
    'String',          '0',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .6 .15 .05]);

% [discFactor]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'discFactor2',...
    'Style',           'edit',...
    'String',          '150',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .55 .15 .05]);

% [SRSdiscFactor]
uicontrol(...
    'Parent',          tab2pan,...
    'Tag',             'SRS_disc2',...
    'Style',           'edit',...
    'String',          '50',...      % initial value
    'Units',           'normalized',...
    'Position',        [.75 .5 .15 .05]);

%run button
uicontrol(...
    'Parent',       tab2pan,...
    'Tag',             'runBtn2',...
    'Style',           'pushbutton',...
    'String',          'Run Simulation',...
    'FontSize',        12,...
    'FontWeight',      'bold',...
    'Units',           'normalized',...
    'Position',        [.45 .05 .15 .1],...
    'Callback',        @run_button2_Callback);

%zoom_box button
uicontrol(...
    'Parent',           tab2pan,...
    'Tag',             'zoomboxBtn2',...
    'Style',           'pushbutton',...
    'String',          'Zoom in on Snells Area',...
    'FontSize',        10,...
    'FontWeight',      'bold',...
    'Units',           'normalized',...
    'Position',        [.75 .45 .15 .05],...
    'Callback',        @zoombox_button2_Callback);

% [axes] - add axes for plot
axes(...
    'Parent',          tab2pan,...
    'Tag',             'axes2',...
    'Units',           'normalized',...
    'Position',        [.05 .4 .5 .5]);

end

function time_slider_Callback(hObject,callbackdata)
    data = guidata(hObject);
    handles = guihandles;
    set(handles.time,'String',num2str(get(handles.time_slider,'Value')));
    run_button_Callback(hObject,callbackdata)
end

function zoombox_button_Callback(hObject,callbackdata)
    data = guidata(hObject);
    handles = guihandles;
    %Gets y0 and sets it as bottom of graph box, -y0 as top
    ystartP = str2double(get(handles.ystartP, 'String')); %y-coordinate of start position
    axis(handles.axes,[ystartP -ystartP ystartP -ystartP])
end

function zoombox_button2_Callback(hObject,callbackdata)
    data = guidata(hObject);
    handles = guihandles;
    %Gets y0 and sets it as bottom of graph box, -y0 as top
    ystartP = str2double(get(handles.ystartP2, 'String')); %y-coordinate of start position
    axis(handles.axes2,[ystartP -ystartP ystartP -ystartP])
end

%need to comment the algorithm
%ultimately want to show feasible set
function run_button_Callback(hObject,callbackdata)
    data = guidata(hObject);
    handles = guihandles;
    axes_handle = handles.axes;
    set(axes_handle,'NextPlot','replacechildren')
    %turn buttons off while running
    allEnabled = findobj(handles.fig,'Enable','on');
    set(allEnabled,'Enable','off');
    
    %grab the old value of the button string
    %change string to running
    set(hObject, 'String', 'Running...');
    drawnow;
    
    %get the input arguments
    r1 = str2double(get(handles.speed1, 'String')); %speed in first region
    r2 = str2double(get(handles.speed2, 'String')); %speed in second region
    ystartP = str2double(get(handles.ystartP, 'String')); %y-coordinate of start position
    time = str2double(get(handles.time, 'String')); %time to travel
    discFactor = str2double(get(handles.discFactor, 'String')); %amount to discretize
    SRS_disc = str2double(get(handles.SRS_disc, 'String')); %amount to discretize
    
    %get the current figure
%     curFig = gcf;
    cla;
    angles = linspace(0,2*pi,discFactor);
    critAng = asin(r1/r2);
    if r2>r1
        angles = [angles pi/2-critAng pi/2+critAng];
    end
    maxDist = max(r1,r2)*time;
    startBL = -maxDist;
    endBL = maxDist;
    plot(axes_handle,[startBL endBL],[0 0],'k'); axis equal;
    hold(axes_handle);

    for i = 1:length(angles)
        calculate(r1,r2,ystartP,time,angles,i,critAng,SRS_disc,axes_handle)
    end %end for loop
        
    % reenable all gui elements
    set(allEnabled,'Enable','on');
    set(hObject, 'String','Run Simulation');
end

% callback for ellipse
function run_button2_Callback(hObject,callbackdata)
    data = guidata(hObject);
    handles = guihandles;
    axes_handle = handles.axes2;
    set(axes_handle,'NextPlot','replacechildren')
    %turn buttons off while running
    allEnabled = findobj(handles.fig,'Enable','on');
    set(allEnabled,'Enable','off');
    
    %grab the old value of the button string
    %change string to running
    set(hObject, 'String', 'Running...');
    drawnow;
    
    %get the input arguments
    a1 = str2double(get(handles.a1, 'String')); % vertical distance from center to ellipse in first region
    b1 = str2double(get(handles.b1, 'String')); % horizontal distance from center to ellipse in first region
    a2 = str2double(get(handles.a2, 'String')); % vertical distance from center to ellipse in second region
    b2 = str2double(get(handles.b2, 'String')); % horizontal distance from center to ellipse in second region
    ystartP = str2double(get(handles.ystartP2, 'String')); % y-coordinate of start position
    time = str2double(get(handles.time2, 'String')); % time to travel
    discFactor = str2double(get(handles.discFactor2, 'String')); % amount to discretize for Snell's Law
    SRS_disc = str2double(get(handles.SRS_disc2, 'String')); % amount to discretize for SRS paths
    
    %get the current figure
%     curFig = gcf;
    cla;
    angles = linspace(0,2*pi,discFactor);
    critAng = asin(a1/a2);
    if a2>a1
        angles = [angles pi/2-critAng pi/2+critAng];
    end
    maxDist = max([a1 b1 a2 b2])*time;
    startBL = -maxDist;
    endBL = maxDist;
    plot(axes_handle,[startBL endBL],[0 0],'k'); axis equal;
    hold(axes_handle);
    
    %angles = pi/2-angles;
    
    for i = 1:length(angles)
        calculate2(a1,b1,a2,b2,ystartP,time,angles,i,critAng,SRS_disc,axes_handle)
    end %end for loop
        
    % reenable all gui elements
    set(allEnabled,'Enable','on');
    set(hObject, 'String','Run Simulation');
end

function calculate(r1,r2,ystartP,time,angles,i,critAng,SRS_disc,axes_handle)
    lineEndY = r1*time*sin(angles(i))+ystartP;
    if lineEndY>0
        %then we touch the boundary
        %t1 = time in region 1
        t1 = sqrt((abs(ystartP)/tan(angles(i)))^2+ystartP^2)/r1;
        %t2 = time in region 2
        t2 = time-t1;
        %x2 = x coordinate of end point
        x2 = (abs(ystartP)/tan(angles(i)))*(r2^2)*t2/(sqrt((abs(ystartP)/tan(angles(i)))^2+ystartP^2)*r1)+(abs(ystartP)/tan(angles(i)));
        %y2 = y coordinate of end point
        y2 = sqrt((r2*t2)^2-(x2-(abs(ystartP)/tan(angles(i))))^2);
        if real(y2) > 0
            plotTrajects(0,abs(ystartP)/tan(angles(i)),ystartP,0,'b',axes_handle)
            plotTrajects(abs(ystartP)/tan(angles(i)),x2,0,y2,'m',axes_handle)
        end
        if angles(i) == pi/2-critAng && r2>r1
            disc = linspace(abs(ystartP)/tan(angles(i)),x2,SRS_disc);
            
            for j = 1:length(disc)
                t3 = time - (t1+(disc(j)-(abs(ystartP)/tan(angles(i))))/r2);
                plotTrajects(disc(j),disc(j)+sin(critAng)*t3*r1,0,-t3*r1*cos(critAng),'r',axes_handle)
            end
        elseif angles(i) == pi/2+critAng && r2>r1
            disc_dist = abs(ystartP)/tan(angles(i));
            disc = linspace(x2,disc_dist,SRS_disc);
            for j = 1:length(disc)
                t3 = time - (t1+abs(disc(j)-disc_dist)/r2);
                plotTrajects(disc(j),disc(j)-sin(critAng)*t3*r1,0,-t3*r1*cos(critAng),'r',axes_handle)
            end
        end


    else
        %then we don't touch the boundary
        totDist = r1*time;
        x = totDist*cos(angles(i));
        %whether or not term 2 in y is negative depends on angle
        y = ystartP + totDist*sin(angles(i));
        plotTrajects(0,x,ystartP,y,'b',axes_handle)
    end %end if statement 
end

function calculate2(a1,b1,a2,b2,ystartP,time,angles,i,critAng,SRS_disc,axes_handle)
    
    % x component of velocity vector in region 1
    x1_vel = a1*b1/sqrt(b1^2+a1^2*tan(angles(i))^2);
    % velocity in region 1
    r1 = x1_vel/abs(cos(angles(i)));
    
    lineEndY = r1*time*sin(angles(i))+ystartP;
    if lineEndY>0
        % out angle from Snell's Law for ellipses
        theta2 = findTheta2(a1, a2, angles(i));
        % x component of velocity vector in region 2
        x2_vel = a2*b2/sqrt(b2^2+a2^2*tan(theta2)^2);
        % velocity in region 2
        r2 = x2_vel/abs(cos(theta2));
        % t1 = time in region 1
        t1 = sqrt((abs(ystartP)/tan(angles(i)))^2+ystartP^2)/r1;
        % t2 = time in region 2
        t2 = time-t1;
        % x2 = x coordinate of end point
        x2 = (abs(ystartP)/tan(angles(i)))*(r2^2)*t2/(sqrt((abs(ystartP)/tan(angles(i)))^2+ystartP^2)*r1)+(abs(ystartP)/tan(angles(i)));
        % y2 = y coordinate of end point
        y2 = sqrt((r2*t2)^2-(x2-(abs(ystartP)/tan(angles(i))))^2);
        if real(y2) > 0
            plotTrajects(0,abs(ystartP)/tan(angles(i)),ystartP,0,'b',axes_handle)
            plotTrajects(abs(ystartP)/tan(angles(i)),x2,0,y2,'m',axes_handle)
        end
        if angles(i) == pi/2-critAng && r2>r1
            disc = linspace(abs(ystartP)/tan(angles(i)),x2,SRS_disc);
            
            for j = 1:length(disc)
                t3 = time - (t1+(disc(j)-(abs(ystartP)/tan(angles(i))))/r2);
                plotTrajects(disc(j),disc(j)+sin(critAng)*t3*r1,0,-t3*r1*cos(critAng),'r',axes_handle)
            end
        elseif angles(i) == pi/2+critAng && r2>r1
%             disc_dist = xstartP+abs(ystartP)/tan(angles(i));
            disc_dist = abs(ystartP)/tan(angles(i));
            disc = linspace(x2,disc_dist,SRS_disc);
            for j = 1:length(disc)
                t3 = time - (t1+abs(disc(j)-disc_dist)/r2);
                plotTrajects(disc(j),disc(j)-sin(critAng)*t3*r1,0,-t3*r1*cos(critAng),'r',axes_handle)
            end
        end


    else
        %then we don't touch the boundary
        totDist = r1*time;
%         x = xstartP + totDist*cos(angles(i));
        x = totDist*cos(angles(i));
        %whether or not term 2 in y is negative depends on angle
        y = ystartP + totDist*sin(angles(i));
        plotTrajects(0,x,ystartP,y,'b',axes_handle)
    end %end if statement 
end

function plotTrajects(x0,x1,y0,y1,color,axes_handle)
    plot(axes_handle,[x0 x1],[y0 y1],color); axis equal;
end
    
function theta2 = findTheta2(a1, a2, theta1)
    theta2 = asin(a1/a2*sin(theta1));
end
    

