...
function run_button_Callback(hObject,callbackdata)
    ...
    points = linspace(-x1max,x1max,100);
    ...
    for i = 1:length(points)
        ...
        R = x2max;  %or whatever radius you want
    ...
    normalAngle = acos(abs(ystartP)/(r1*time));
    angles = linspace( pi/2 + normalAngle, 5*pi/2 - normalAngle, 100);
    feasibleDist = r1*time;
    x = feasibleDist*cos(angles) + xstartP;
    y = feasibleDist*sin(angles) + ystartP;
    plot(x,y);
    hold on;
    %plot the boundary line between blue and brown regions
    boundary = zeros(1,11);
    plot(-1:0.2:1,boundary)
    hold on;
    plot(boundary,-1:0.2:1)
